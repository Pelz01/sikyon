...
const records = [
  {
    ...
    status: "Verified",
    auditorSignature: "0xAUD...9F1",
    blobId: "wal_0x8f7c9e0d1a2938afbc9e",
    objectId: "0x9d1b7f4a8c2e6b019af5c31d8e72a449",
    hash: "91b7f8c0d4c1e2a53f6a7d1b0c9e8f37452aa7016cfbd8e9f0a142b3c4d5e6f71",
  },
  {
    attestationId: "attestation-002",
    ...
    hash: "c2e4f6a8190b73dd45f9a2c18e7b61f0d3a55c9e48217a0bb6f33d2e1c984af5",
  },
  {
    attestationId: "attestation-003",
    title: "Q2 Corporate Cash Reserve Statement",
    institution: "Coinbase Prime",
    status: "Verified",
    auditorSignature: "0xAUD...4A2",
...
export function generateStaticParams() {
  return records.map((record) => ({ attestationId: record.attestationId }));
}
...
export default async function VerifyPage({
  ...
  const record = records.find((item) => item.attestationId === attestationId);
    ...
    <main className="min-h-screen bg-zinc-50 px-6 pb-20 pt-28 text-zinc-950">
      <section className="mx-auto max-w-3xl rounded-lg border border-zinc-200 bg-white p-6 shadow-sm md:p-8">
        <div className="flex items-center gap-3 border-b border-zinc-200 pb-6">
          ...
          <div>
            <p className="text-sm font-medium text-emerald-600">Verification Status: {record.status}</p>
            <h1 className="mt-1 text-2xl font-medium tracking-tight text-zinc-950">{record.title}</h1>
            <p className="mt-2 font-mono text-xs text-zinc-500">/{record.attestationId}</p>
          </div>
        </div>
        ...
        <div className="grid gap-3 py-6 text-sm">
          {[
            ...
            ["Hash", record.hash],
            ["Auditor signature", record.auditorSignature],
